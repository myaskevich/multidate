...
function parser(str, offset, multi) {
  ...
  while (advance !== 'eos') {
    if (multi === true && this.date.dirty) {
      this.dates.push(new date(before));
      this.date.dirty = false;

      delta = advance;
      // if (months.indexOf(delta) !== -1) delta = 'month';
      // if (days.indexOf(delta) !== -1) delta = 'day';
      // if (rDayMod.test(delta)) delta = 'hour';

      delete this.date._changed[delta];
    }
