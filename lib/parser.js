...
var rMonthsOf = /^((\d{1,2})(st|nd|rd|th)?)\sof\s(january|february|march|april|may|june|july|august|september|october|november|december)/;
var rMonthsLeft = /^(january|february|march|april|may|june|july|august|september|october|november|december)\s(\d{1,2})(st|nd|rd|th)?/;
...
function parser(str, offset, multi) {
  ...
  while (advance !== 'eos') {
    ...
    if (multi === true && Object.keys(changed).length) {
      if (Object.keys(changed).indexOf('day') > -1 && Object.keys(this.date._changed).indexOf('hour') > -1) {
        this.dates.push(new date(before));
...
parser.prototype.monthByName = function() {
  var captures;
  if (captures = rMonthsOf.exec(this.str)) {
    var day = captures[2]
    var month = captures[4];
  } else if (captures = rMonthsLeft.exec(this.str)) {
    var day = captures[2]
